<script lang="ts">
	...
	let targetDate = new Date('2025-02-08T10:00:00');
	...
	const dayTitles = {
		...
		1: 'Flamman Pub',
		2: 'Türolerröjet',
		3: 'Temakväll på Flamman',
		4: 'Valla Saucer Rennen'
	};

	const openDayModal = (day: 0 | 1 | 2 | 3 | 4) => {
...
{#if scrollY >= 2000 && scrollY < 5000}
	<div class="fixed z-10 inset-0 overflow-hidden">
		<div class="absolute top-1/2 left-1/2">
			...
			>
				<div style="opacity: {Math.min(0.5, clockOpacity)};" class="-mb-2">5-8.2.2025</div>
...
<div class="flex flex-col gap-4 items-center w-full justify-center sticky top-12 sm:top-12">
	...
	<div class="px-2 sm:w-1/2 text-center font-medium text-2xl sm:text-3xl">
		...
		{:else if daysLeft === 0}
			<span class="font-bold"> Idag smäller det! </span>
		{:else}
			<span class="font-bold"> Tack för i år! </span>
		{/if}
	</div>
	<div class="grid grid-cols-2 gap-4 w-full px-2 sm:px-0 font-bold sm:w-1/2">
		...
		<button
			...
			type="button">Tordag 1.31</button
		>
		<button
			on:click={() => openDayModal(1)}
			class="btn w-full variant-outline-primary"
			type="button">Onsdag 5.2</button
		>
		<button
			on:click={() => openDayModal(2)}
			class="btn w-full variant-outline-primary"
			type="button">Torsdag 6.2</button
		>
		<button
			on:click={() => openDayModal(3)}
			class="btn w-full variant-outline-primary"
			type="button">Fredag 7.2</button
		>
		<button
			on:click={() => openDayModal(4)}
			class="btn w-full variant-outline-primary"
			type="button">Lördag 8.2</button
		>
		<a href="/tavling" class="btn variant-filled-primary text-center w-full col-span-2 font-bold"
			>Öppna Tävling</a
		>
	</div>
